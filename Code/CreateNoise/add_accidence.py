...
out = '../Result/accident/miu=' + str(Miu) + '_sigma=' + str(Sigma) + '/simulate_data/'
...
for exp in range(start, end + 1):
    ...
    df = pd.read_csv('../Result/origin/miu=' + str(Miu) + '_sigma=' + str(Sigma) + '/simulate_data/experiment' + str(exp) + '/data.csv')
    ...
    col_name.insert(index, 'with accident')
    df = df.reindex(columns=col_name)
    df['with accident'] = [int(0)] * df.shape[0]
...
with open('../Result/accident/miu=' + str(Miu) + '_sigma=' + str(Sigma) + '/result.csv', 'w', newline='',
